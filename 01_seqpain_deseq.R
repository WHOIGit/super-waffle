...
num_threads <- BiocParallel::multicoreWorkers()
cluster <- multidplyr::new_cluster(n = (num_threads - 2))
...
#This experiement looked at the differential expression in treated cells from Breast Cancer
# and from Nociceptor cell lines
#whether the addition of Kw, the addition of Taxol, and/or the addition of Kw+Taxol
#impacted expression compared to a control (the DMSO-treated cells)
...
counts_combined <- read_tsv(file = "https://github.com/labsyspharm/seqpain/raw/main/data/counts_combined.tsv.gz",
                            col_names = TRUE,
                            name_repair = "unique",
                            trim_ws = TRUE)

#import metadata
rnaseq_metadata <- read_tsv(
  file = "https://github.com/labsyspharm/seqpain/raw/main/data/rnaseq_metadata.tsv",
  # file = paste0(dir, "/data/rnaseq_metadata.tsv"),
                            col_names = TRUE,
                            name_repair = "unique",
                            trim_ws = TRUE) %>%
...
write_tsv(counts_rep_protcod_df, 
          file = paste0(dir, "/data/counts_rep_protcod_df.tsv.gz"), 
          quote = "none", 
          col_names = TRUE)

#import the counts files
# counts_rep_protcod_df <- read_tsv(file = paste0(dir, "/data/counts_rep_protcod_df.tsv.gz"),
#                             col_names = TRUE,
#                             trim_ws = TRUE,
#                             name_repair = "unique")


counts_rep_protcod_df <- readr::read_delim(file = paste0(dir, "/data/counts_rep_protcod_df.tsv.gz"), 
...
res_nociceptor_small <- F_extract_results_multi(deseq_nociceptor_small, 0.05)
